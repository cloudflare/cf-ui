...
describe('parallel()', () => {
  ...
  it('should call the next function before finishing the last one', done => {
    ...
    parallel([
      ...
      cb => {
        assert.ok(!called, 'second callback was not called before first function was complete');
        cb();
      }
    ], done);
  });
  ...
  it('should call all the functions in the parallel even when one gets an error synchronously', done => {
    let called = false;
    parallel([
      cb => cb(new Error('Oh no!')),
      cb => (called = true, cb())
    ], () => {});
